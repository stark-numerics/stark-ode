...
from stark.carriers import CarrierJax  # noqa: E402
...
def test_jax_interface_smoke_state_translation_and_derivative():
    carrier = CarrierJax(jnp.array([1.0, 2.0]))
...
def test_stark_ivp_prepares_jax_initial_with_jax_carrier():
    initial = jnp.array([1.0, 2.0])
    carrier = CarrierJax(initial)
    ...
    assert ivp.prepared_carrier.arithmetic.preference == "return"
