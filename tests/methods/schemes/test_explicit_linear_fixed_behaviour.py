...
def test_fixed_explicit_linear_fixed_path_matches_inline_semantics() -> None:
    inline_state = build_state()
    linear_fixed_state = build_state()

    inline = SchemeRK4(DummyArrayDynamics(), DummyArrayAllocator(3))
    specialized = SchemeRK4(
        DummyArrayDynamics(),
        DummyArrayAllocator(3),
        linear_fixed=DummyArrayLinearFixed(),
    )
    ...
    linear_fixed_dt = specialized(
        Interval(present=0.0, step=0.05, stop=0.05),
        linear_fixed_state,
    )

    assert linear_fixed_dt == pytest.approx(inline_dt)
    np.testing.assert_allclose(
        linear_fixed_state.y,
        inline_state.y,
        rtol=1.0e-14,
        atol=1.0e-14,
    )
...
def test_adaptive_explicit_linear_fixed_path_matches_inline_semantics() -> None:
    ...
    assert linear_fixed_next_step == pytest.approx(
        inline_next_step,
        rel=1.0e-14,
        abs=1.0e-14,
    )
    ...
    linear_fixed_report = specialized_scheme.step_control.report()

    assert linear_fixed_report.accepted_dt == pytest.approx(
        inline_report.accepted_dt,
        rel=1.0e-14,
        abs=1.0e-14,
    )
    assert linear_fixed_report.proposed_dt == pytest.approx(
        inline_report.proposed_dt,
        rel=1.0e-14,
        abs=1.0e-14,
    )
    assert linear_fixed_report.next_dt == pytest.approx(
        inline_report.next_dt,
        rel=1.0e-14,
        abs=1.0e-14,
    )
    assert linear_fixed_report.error_ratio == pytest.approx(
        inline_report.error_ratio,
        rel=1.0e-14,
        abs=1.0e-14,
    )
    assert linear_fixed_report.rejection_count == inline_report.rejection_count
...
def test_inline_scheme_does_not_carry_generated_source_state() -> None:
