...
def test_native_scalar_rms() -> None:
    norm = DeprecatedCarrierNormNativeRMS().bind(template=1.0)
...
def test_native_empty_list_rms_is_zero() -> None:
    norm = DeprecatedCarrierNormNativeRMS().bind(template=[])
...
def test_numpy_max() -> None:
    ...
    value = np.array([-3.0, 4.0])
    norm = DeprecatedCarrierNormNumpyMax().bind(template=value)
