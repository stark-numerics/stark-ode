...
def make_scheme() -> SchemeKennedyCarpenter54:
...
def test_kennedy_carpenter54_owns_converted_call_surface() -> None:
    ...
    assert "call_inline" in SchemeKennedyCarpenter54.__dict__
    assert "call_specialized" in SchemeKennedyCarpenter54.__dict__
...
def test_kennedy_carpenter54_clips_to_remaining_interval() -> None:
    scheme = make_scheme()
...
def test_kennedy_carpenter54_monitoring_uses_scheme_owned_boundary() -> None:
    scheme = make_scheme()
    monitor = Monitor()
    interval = Interval(present=0.0, step=0.1, stop=0.3)
    state = ScalarState(2.0)
