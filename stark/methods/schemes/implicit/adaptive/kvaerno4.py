...
class SchemeKvaerno4:
    ...
    def __init__(self, dynamics: DynamicsLike, allocator: AllocatorLike, resolvent: Resolvent, *, configuration: SchemeConfiguration | None = None, linear_fixed: SchemeLinearFixedLike | None = None, monitor: SchemeMonitor | None = None) -> None:
    ...
    def prepare_specialized_kernels(self, linear_fixed: SchemeLinearFixedLike) -> None:
        self.known2_call = linear_fixed(SchemeStencil((1.0,), scale=KVAERNO4_GAMMA))
        self.known3_call = linear_fixed(SchemeStencil(_KNOWN3_WEIGHTS))
        self.known4_call = linear_fixed(SchemeStencil(_KNOWN4_WEIGHTS))
        self.known5_call = linear_fixed(SchemeStencil(_KNOWN5_WEIGHTS))
    ...
    def call_inline(self, interval: IntervalLike, state: State) -> float:
        ...
        while True:
            ...
            try:
                delta2 = self._solve_stage(interval, state, dt, KVAERNO4_C2 * dt, dt * KVAERNO4_GAMMA, delta1, self.known2_block, self.delta2_block, previous=delta1)
                known3 = combine2(
                    _KNOWN3_WEIGHTS[0],
                    delta1,
                    _KNOWN3_WEIGHTS[1],
                ...
                delta3 = self._solve_stage(interval, state, dt, KVAERNO4_C3 * dt, dt * KVAERNO4_GAMMA, known3, self.known3_block, self.delta3_block, previous=delta2)
                known4 = combine3(
                    _KNOWN4_WEIGHTS[0],
                    delta1,
                    _KNOWN4_WEIGHTS[1],
                ...
                delta4 = self._solve_stage(interval, state, dt, dt, dt * KVAERNO4_GAMMA, known4, self.known4_block, self.delta4_block, previous=delta3)
                known5 = combine4(
                    _KNOWN5_WEIGHTS[0],
                    delta1,
                    _KNOWN5_WEIGHTS[1],
                ...
                delta5 = self._solve_stage(interval, state, dt, dt, dt * KVAERNO4_GAMMA, known5, self.known5_block, self.delta5_block, previous=delta4)
