...
from stark.methods.inverters.legacy_support import (
...
class InverterBiCGStab:
    ...
    def __init__(
        ...
        # Installs self.workspace; see stark.methods.inverters.legacy_support.workspace for its operations.
        initialise_inverter_runtime(
            self,
            allocator,
            inner_product,
    ...
    def iterate(self, out: Block, operator: BlockOperatorDiagonal, rhs_norm: float) -> tuple[int, float]:
        ...
        for iteration in range(1, policy.max_iterations + 1):
            ...
            if inverter_tolerance.accepts(workspace.norm(self.s_buffer), rhs_norm):
