...
from stark.algebraist.codegen import field_apply_assignment, field_combine_assignment, field_norm_body
from stark.algebraist.explicit import AlgebraistExplicitSchemeBinder
from stark.algebraist.fields import AlgebraistField
from stark.algebraist.names import combine_kernel_name, combine_wrapper_name, linear_combine_names
...
class Algebraist:
    ...
    def make_combine_kernel(self, term_count: int) -> tuple[str, Callable[..., object], str]:
        ...
        field_arguments = [f"out_{field.translation_name}" for field in self.fields]
    ...
    def make_combine_wrapper(self, term_count: int) -> tuple[str, Callable[..., object], str]:
        wrapper_name = combine_wrapper_name(term_count)
        kernel_name = combine_kernel_name(term_count)

        parameters = ["out"]
        kernel_arguments = [path_expression("out", field.translation_path) for field in self.fields]
        for index in range(term_count):
            parameters.append(f"a{index}")
            parameters.append(f"x{index}")
            kernel_arguments.append(f"a{index}")
            kernel_arguments.extend(path_expression(f"x{index}", field.translation_path) for field in self.fields)
        source = (
    ...
    def make_apply_kernel(self) -> tuple[str, Callable[..., object], str]:
        ...
        origin_arguments = [f"origin_{field.state_name}" for field in self.fields]
        delta_arguments = [f"delta_{field.translation_name}" for field in self.fields]
        result_arguments = [f"result_{field.state_name}" for field in self.fields]
    ...
    def make_apply_wrapper(self) -> tuple[str, Callable[..., object], str]:
        arguments = [path_expression("origin", field.state_path) for field in self.fields]
        arguments.extend(path_expression("translation", field.translation_path) for field in self.fields)
        arguments.extend(path_expression("result", field.state_path) for field in self.fields)
        source = (
    ...
    def make_norm_wrapper(self) -> tuple[str, Callable[..., object], str]:
        arguments = ", ".join(path_expression("translation", field.translation_path) for field in self.fields)
        source = (
