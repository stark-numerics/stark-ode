...
RKF45_ERROR_WEIGHTS = tuple(
    high - low
    for high, low in zip(RKF45_B_HIGH, RKF45_B_LOW, strict=True)
)
...
class SchemeFehlberg45:
    ...
    Fehlberg's method is one of the classic adaptive explicit Runge-Kutta constructions: a fifth-order update paired with a fourth-order estimate for step control.
    ...
    def call_inline(
        ...
        while True:
            ...
            derivative(stage_interval(interval, dt, dt * RKF45_C[1]), stage, k2)
            ...
            stage_delta(state, stage)
            derivative(stage_interval(interval, dt, dt * RKF45_C[2]), stage, k3)
            ...
            stage_delta(state, stage)
            derivative(stage_interval(interval, dt, dt * RKF45_C[3]), stage, k4)
            ...
            stage_delta(state, stage)
            derivative(stage_interval(interval, dt, dt * RKF45_C[4]), stage, k5)
            ...
            stage_delta(state, stage)
            derivative(stage_interval(interval, dt, dt * RKF45_C[5]), stage, k6)
            # 7. high_delta = h * b_high dot k
            high_delta = combine5(
