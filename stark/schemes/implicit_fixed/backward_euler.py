...
class SchemeBackwardEuler(SchemeBaseImplicitFixed):
    ...
    so each step requires a shifted implicit solve rather than a direct
    explicit stage evaluation. In STARK that solve is owned by a `Resolvent`,
    which may be a generic automatic worker or a problem-specific custom one.
    ...
    def __init__(
        ...
        self.stepper = ShiftedOneStageResolventStep("Backward Euler", self.tableau, derivative, workbench, resolvent)
    ...
    def __call__(self, interval: IntervalLike, state: State, executor: Executor) -> float:
        ...
        delta = self.stepper.solve(interval, state, dt, alpha=dt, stage_shift=dt)
        self.stepper.workspace.apply_delta(delta, state)
