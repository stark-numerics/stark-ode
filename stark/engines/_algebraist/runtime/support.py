...
@dataclass(slots=True)
class AlgebraistRuntimeUnitApplyKernel(Generic[StateType, TranslationType]):
    """Runtime state-apply kernel for an already computed translation."""
...
class AlgebraistRuntimeSupport(Generic[TranslationType]):
    ...
    def provide_linear_fixed(self, request: AlgebraistStencil) -> Callable[..., object]:
    ...
    def provide_unit_apply(self) -> Callable[..., State]:
        kernel = AlgebraistRuntimeUnitApplyKernel[State, TranslationType]()
        return self.accelerate(kernel, label="runtime.apply.unit", arity=1)
...
__all__ = [
    ...
    "AlgebraistRuntimeUnitApplyKernel",
