...
This private package preserves the old algebra implementation while remaining
dependencies are audited. It is not part of the public engine surface.
...
```text
Frame-backed state  -> Generator kernels             -> preferred accelerated path
foreign state       -> allocator runtime helpers     -> flexible low-level path
```

Do not treat `_algebraist` runtime code as the default high-level path for
ordinary `Frame` models.
...
Frame
  ...
  -> prepared Generator and allocator hooks
...
behaviour, accelerator, and generator policy. It should not manually
orchestrate every generated provider. `Generator` owns request-driven generated
kernels; allocator decorators own runtime completion for foreign/custom
allocation shapes.
...
norms
inner products
fixed-linear stage updates
```

This is where backend acceleration should happen for ordinary high-level use.

Concrete engines expose `engine.generator`. Downstream code that needs
fixed-linear, norm, inner-product, or linear-combine kernels should call
`engine.generator` with a request. `SchemeStencil`, relaxation stencils, and
resolvent block stencils all satisfy the linear-fixed request shape.
...
Generator      owns request-driven prepared kernels for that layout
Algebraist     temporarily owns unmigrated allocator-facing hooks
