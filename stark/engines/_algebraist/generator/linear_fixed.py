...
class AlgebraistGeneratorLinearFixed(Generic[StateType, TranslationType]):
    ...
    def source_string(self, stencil: AlgebraistStencil) -> str:
        source = getattr(self.target, "source_linear_fixed", None)
        if callable(source):
            return cast(str, source(self.frame, stencil))
        return AlgebraistGeneratorEmitter(self.frame, target=self.target).linear_fixed(stencil)
...
__all__ = ["AlgebraistGeneratorLinearFixed"]
