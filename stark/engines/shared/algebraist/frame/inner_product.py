...
def _pairs(left: Any, right: Any) -> Iterable[tuple[Any, Any]]:
...
class AlgebraistInnerProductL2:
    ...
    def __call__(self, left, right) -> float:
        total: Any = 0.0
        for left_item, right_item in _pairs(left, right):
            total += left_item * right_item
        return float(total)


@dataclass(frozen=True, slots=True)
class AlgebraistInnerProductRMS:
...
@dataclass(frozen=True, slots=True)
class AlgebraistInnerProductExcluded:
