...
from stark.engines.carriers.native.tuple.storage import CarrierNativeTupleValue


@dataclass(frozen=True)
class CarrierArithmeticNativeTuple:
    @property
    ...
    def translate(
        self,
        state: CarrierNativeTupleValue,
        step: float,
        derivative: CarrierNativeTupleValue,
        result: CarrierNativeTupleValue,
    ) -> CarrierNativeTupleValue:
        return tuple(item + step * delta for item, delta in zip(state, derivative))

    def add(
        self,
        left: CarrierNativeTupleValue,
        right: CarrierNativeTupleValue,
        result: CarrierNativeTupleValue,
    ) -> CarrierNativeTupleValue:
    ...
    def scale(
        self,
        factor: float,
        value: CarrierNativeTupleValue,
        result: CarrierNativeTupleValue,
    ) -> CarrierNativeTupleValue:
        return tuple(factor * item for item in value)

    def _combine(self, coefficients: tuple[float, ...], values: tuple[CarrierNativeTupleValue, ...]) -> CarrierNativeTupleValue:
        length = len(values[0])
        return tuple(
