...
## Redirect call paths keep optional work out of hot loops

Schemes, resolvents, and inverters should generally expose a tiny `__call__`
that delegates to a prepared call path:

```python
def __call__(self, request, output):
    return self.redirect_call(request, output)
```

Construction or preparation chooses `redirect_call` once:

```text
unmonitored direct path
monitored wrapper
specialized kernel path
debug/diagnostic wrapper
```

This is not ceremony. It keeps optional functionality from adding branches to
every hot iteration.

Prefer this shape:

```text
__init__ chooses redirect_call
__call__ delegates
call_inline does the plain work
call_specialized does the prepared fast path
call_monitored wraps and records
```

Avoid this shape inside hot loops:
...
A feature example should answer one question. Longer teaching stories should be
split into focused examples before they become part of the release surface.
